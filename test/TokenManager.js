...
contract("TokenManager", async (accounts) => {
    ...
    it("should create and retrieve tokens", async () => {
        await tokMgr.newToken(tokName, "e", 4, role.address, {from: a0});
    ...
    it("fails on duplicated names", async () => {
        let tokName = "eEUR";
        await tokMgr.newToken(tokName, "e", 4, role.address, {from: a0});
        await util.assertReverts(tokMgr.newToken(tokName, "e", 4, role.address, {from: a0}));
    });
...
contract("Token manager list retrieve", async (accounts) => {
    ...
    it("Retrieves a list of created tokens", async () => {
        ...
        await tokMgr.newToken("tok1", "e", 4, role.address, {from: a0});
        await tokMgr.newToken("tok2", "e", 4, role.address, {from: a0});
